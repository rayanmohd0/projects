...
def outcome_handler(user_choice):
    ...
    player_choice_label.config(fg="green", text="you chose " + str(user_choice))
    computer_choice_label.config(fg="red", text="Computer Chose " + str(computer_choice))

    if outcome == 2:
        player_score = player_score + 1
        player_score_label.config(text="you : " + str(player_score))
        outcome_label.config(fg="green", text="Outcome : you Won :)")
    elif outcome == 0:
...
# Main Screen
master = Tk()
master.title("GAMEBOI")

# Labels
Label(master, fg="orange", text="Rock, Paper or Scissor?", font=("Calibri", 22)).grid(row=0, sticky=N, pady=10, padx=200)
Label(master, fg="cyan", text="chose your option : ", font=("Calibri", 14)).grid(row=1, sticky=N)
player_score_label = Label(master, text="you : 0", font=("Calibri", 12))
